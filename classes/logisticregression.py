...
class LogisticRegression:
	...
	def __init__(self, args):
		self.args = args
		self.read_csv(self.args.datafile)
		self.activation = self.sigmoid
	
	def check_input(self, df, features):
		if features == []:
			display_error("Missing features for our training")
		if self.y.any() == False or sum(pd.isnull(df["Hogwarts House"])):
	...
	def read_csv(self, datafile, fill = True):
		...
		try:
			...
			features = list(df.columns[6:]) ### TODO: rajouter option features et aussi ameliorer gestion d'erreur
	...
	def save_weights(self):
		info = {"thetas" : self.thetas, "scaler" : self.scaler}
