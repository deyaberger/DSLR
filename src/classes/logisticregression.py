...
class LogisticRegression:
	...
	def read_csv(self, datafile, train = True):
		...
		try:
			...
			if df.empty == True:
				display_error(f"Error in {self.read_csv.__name__}:\nEmpty DataFrame")
			self.features = []
			if train == True:
		...
		except (FileNotFoundError, errors.EmptyDataError, IndexError) as e:
			display_error(f"Error in {self.read_csv.__name__}:\n{e}")
		except KeyError as e:
			display_error(f"Error in {self.read_csv.__name__}:\nThe csv file does not contain the expected column {e}")
	...
	def hypothesis(self, X):
		...
		try:
			...
			display_error(f"Error in function {self.hypothesis.__name__}")
	...
	def predict(self, y, i):
		...
		try:
			...
			display_error(f"Error in function {self.predict.__name__}")
	...
	def fit(self, score):
		...
		try:
			...
			display_error(f"Error in function {self.fit.__name__}")
	...
	def save_weights(self, file_name):
		try:
			...
			display_error(f"Error in function {self.save_weights.__name__}")
